...
int main(int argc, char* argv[]) {
    ...
    cudaError_t err = cudaMallocHost((void**)&h_buffer, bufferSize);
    ...
    while (fin.read(reinterpret_cast<char*>(h_buffer), bufferSize) || fin.gcount() > 0) {
        ...
        encryptCtr<<<numBlocks, BLOCK_SIZE>>>(d_buffer, d_buffer, bytesRead, totalAESBlocks, (chunkIndex * (BUFFER_SIZE / AES_BLOCK_SIZE)));
        chunkIndex++;
